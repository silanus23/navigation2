...
static constexpr double search_window = 0.325;
...
{
  node_->declare_parameter("PathHugCritic.search_window", search_window);
  EXPECT_EQ(
    node_->get_parameter("PathHugCritic.search_window").as_double(),
    search_window);
}
...
TEST_F(PathHugCriticTest, CustomParameterValues)
{
  ...
  nav2::declare_parameter_if_not_declared(
    custom_node, name + ".search_window",
  ...
  EXPECT_EQ(
    custom_node->get_parameter(name + ".search_window").as_double(),
    custom_distance);
}
